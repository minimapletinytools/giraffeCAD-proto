...
In addition, also run the the type checker
...
4. create a sensible `marking_transform : Transform` on that timber which is a transform from which we will make all further calculations from. Depending on the situation, it may either be in local or global coordiantes (if local postfix with _<timber_name>_local)
    ...
    - for marking transforms inside the timber that are pointing in the length or -length direction, the "+x" axis of the transform should line up with the timber's "+x" axis
        - TODO decide if Y is rotated or if we use a transform with -1 determinant :O
