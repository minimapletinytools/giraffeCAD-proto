...
class TestCheckTimberOverlapForSpliceJoint:
    ...
    @pytest.mark.skip(reason="TODO: Penetration check logic needs refinement - may need to adjust test or validation function")
    def test_invalid_excessive_penetration_A_through_B(self):
        """Test that excessive penetration of timberA through timberB triggers an error."""
        timber_length = inches(36)
        timber_size = create_v2(inches(4), inches(4))
        
        # TimberA pointing east, very long (extends from 0 to 36 inches)
        timberA = timber_from_directions(
            length=timber_length,
            size=timber_size,
            bottom_position=create_v3(0, 0, 0),
        ...
        # TimberB pointing west, very short, positioned so A's TOP end extends
        # WAY past timberB's opposite (TOP) end
        # TimberB bottom at 20in, pointing west, so top at 20-36=-16in
        # TimberA top at 36in, which is 36-(-16)=52in past timberB's top
        # Since timberB is only 36in long, this should trigger the error
        timberB = timber_from_directions(
            length=timber_length,
            size=timber_size,
        ...
        assert error is not None, f"Expected error for excessive penetration, but got: {error}"
        assert "penetrated" in error.lower() or "too far" in error.lower()
    
    @pytest.mark.skip(reason="TODO: Penetration check logic needs refinement - may need to adjust test or validation function")
    def test_invalid_excessive_penetration_B_through_A(self):
        """Test that excessive penetration of timberB through timberA triggers an error."""
        timber_length = inches(36)
        timber_size = create_v2(inches(4), inches(4))
        
        # TimberA pointing east (bottom at 0, top at 36in)
        timberA = timber_from_directions(
            length=timber_length,
            size=timber_size,
            bottom_position=create_v3(0, 0, 0),
        ...
        # TimberB pointing west, positioned so B's BOTTOM end extends
        # WAY past timberA's opposite (BOTTOM) end
        # TimberB bottom at 82in, pointing west (-X), so top at 82-36=46in
        # When we check BOTTOM of B, that's at 82in
        # TimberA bottom at 0in, so B's bottom is 82in past A's top
        # This is >> timberA's length (36in), so should trigger error
        timberB = timber_from_directions(
            length=timber_length,
            size=timber_size,
            bottom_position=create_v3(inches(82), 0, 0),
            length_direction=create_v3(-1, 0, 0),
            width_direction=create_v3(0, 1, 0),
            name="timberB"
        )
